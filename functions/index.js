...
// New Daily Limits Configuration
const DAILY_SHARP_LIMIT = 30; // Max 30 SHARP per user per day
const MAX_PLAYS_PER_GAME = 2; // Max 2 plays per game per day  
const MAX_SHARP_PER_GAME = 5; // Max 5 SHARP per game per day
const GAME_COOLDOWN = 60 * 60 * 1000; // 1 hour in milliseconds
...
exports.submitScore = functions.https.onCall(async (data, context) => {
  ...
  const { score, playDuration, timestamp, gameType, difficulty } = data;

  // Input validation
  if (!score || !playDuration || !timestamp || !gameType) {
  ...
  // Check daily and game limits
  const today = new Date().toISOString().split('T')[0]; // YYYY-MM-DD format
  
    // Check if user can play this game (cooldown + daily limits)
    const canPlay = await checkGameEligibility(uid, gameType, today);
    if (!canPlay.allowed) {
      throw new functions.https.HttpsError('failed-precondition', canPlay.message);
    }
    // i am six year old
    // i study in UKG class 
  //   i live in vijay nagar, ghaziabad.


    // Enhanced anti-cheat: Check score is realistic for play duration
    const maxPossibleScore = playDuration * 10; // ~10 points per second max
    if (score > maxPossibleScore) {
      throw new functions.https.HttpsError('invalid-argument',
      'Score too high for play duration - possible cheat detected');
  }

  // Check for impossible scores
  if (score > 10000) {
    throw new functions.https.HttpsError('invalid-argument',
      'Score exceeds maximum possible value');
  }
  ...
  try {
    ...
    // Calculate SHARP reward using new system
    const sharpReward = calculateSharpReward(score, gameType, difficulty);

    // Save game score with new fields
    await db.collection('gamescores').add({
      ...
      rewardAmount: sharpReward // Keep for backward compatibility
    });
    ...
    const updateData = {
      ...
      tokensBalance: admin.firestore.FieldValue.increment(sharpReward),
      totalEarned: admin.firestore.FieldValue.increment(sharpReward)
    };
...
exports.getUserStats = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    throw new functions.https.HttpsError('internal', 'Error getting user stats');
  }
});

/**
 * Check if user can play a specific game (cooldown + daily limits)
 */
async function checkGameEligibility(uid, gameType, today) {
  try {
    // Get user's plays for today
    ...
    // Check daily SHARP limit
    const todaySharp = todayPlays.reduce((sum, play) => sum + (play.sharpEarned || 0), 0);
    if (todaySharp >= DAILY_SHARP_LIMIT) {
      return {
        allowed: false,
        message: `Daily limit reached! You've earned ${todaySharp}/${DAILY_SHARP_LIMIT} SHARP today. Come back tomorrow!`
      };
    }
    ...
    // Check max plays per game per day
    if (gamePlaysToday.length >= MAX_PLAYS_PER_GAME) {
      return {
        allowed: false,
        message: `${gameType} daily limit reached! You've played ${gamePlaysToday.length}/${MAX_PLAYS_PER_GAME} times today. Try other games!`
      };
    }
    ...
    if (gamePlaysToday.length > 0) {
      ...
      if (timeDiff < GAME_COOLDOWN) {
        const remainingTime = Math.ceil((GAME_COOLDOWN - timeDiff) / (1000 * 60)); // minutes
        return {
          allowed: false,
          message: `Cooldown active! Wait ${remainingTime} minutes before playing ${gameType} again.`
        };
      }
    }
    
    // Check game SHARP limit
    const gameSharpToday = gamePlaysToday.reduce((sum, play) => sum + (play.sharpEarned || 0), 0);
    if (gameSharpToday >= MAX_SHARP_PER_GAME) {
      return {
        allowed: false,
        message: `${gameType} SHARP limit reached! You've earned ${gameSharpToday}/${MAX_SHARP_PER_GAME} SHARP from this game today.`
      };
    }
    ...
    return { allowed: false, message: 'Error checking eligibility' };
  }
}
 ...
 */
function calculateSharpReward(score, gameType, difficulty) {
  let baseReward = 0;
  
  // Base reward calculation by game type
  switch (gameType) {
    case 'memory':
      baseReward = Math.min(5, Math.floor(score / 2000)); // Max 5 SHARP
      break;
    case 'color-rush':
      baseReward = Math.min(5, Math.floor(score / 1500)); // Max 5 SHARP
      break;
    case 'sharp-shooter':
      baseReward = Math.min(5, Math.floor(score / 1000)); // Max 5 SHARP
      break;
    case 'stack-game':
      baseReward = Math.min(5, Math.floor(score / 800)); // Max 5 SHARP
      break;
    default:
      baseReward = Math.min(5, Math.floor(score / 1000));
  }
  
  // Difficulty multiplier
  const multiplier = difficulty === 'easy' ? 0.8 : difficulty === 'hard' ? 1.2 : 1.0;
  
  // Ensure minimum 1 SHARP for completing a game
 ...
 */
exports.getDailyGameStatus = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
  ...
  try {
    ...
    for (const game of VALID_GAMES) {
      ...
      if (gamePlays.length > 0) {
        ...
        const lastPlayTime = lastPlay.timestamp.toMillis();
        nextPlayTime = lastPlayTime + GAME_COOLDOWN;
      }
      
      gameStatus[game] = {
        playsToday: gamePlays.length,
        maxPlays: MAX_PLAYS_PER_GAME,
        sharpEarned: gameSharp,
        maxSharp: MAX_SHARP_PER_GAME,
        canPlay: gamePlays.length < MAX_PLAYS_PER_GAME && gameSharp < MAX_SHARP_PER_GAME,
    ...
    return {
      ...
      maxDailySharp: DAILY_SHARP_LIMIT,
      canEarnMore: todaySharp < DAILY_SHARP_LIMIT,
      games: gameStatus
    };
    
  } catch (error) {
    console.error('Error getting daily game status:', error);
    throw new functions.https.HttpsError('internal', 'Error getting daily status');
  }
});
